...
int main() {
  ...
  displayResults(tf, out, sampleTimes, samples, "test", tSpline, tSplineB);
  ...
  // // displayResults(tf, out, sampleTimes, samples, "singlePulseFlatNoise",
  // // tSpline);

  // // single pulse arbitrary noise
  // cout << "testing arbitrary sample noise, adding sqrt((s-ped)/50) worth of "
  //         "noise" << endl;
  ...
  //     sampleVariance +=
  //         (samples[s] - pedestal) * (samples[s] - pedestal) / (50 * 50);
  //   }
  ...
  // "singlePulseArbitraryNoise",
  // // tSpline);

  // cout << "testing discontiguous fit regions (clipping) with flat noise: "
  //      << endl;
  // energy *= 4;
  ...
  // }
  // timeGuess =
  //     max_element(goodSamples.begin(), goodSamples.end()) -
  //     goodSamples.begin();
  // out = tf.discontiguousFit(goodSamples, times, timeGuess, noise);
  // // displayResults(tf, out, times, goodSamples, "clippedFitWithFlatErrors",
  // // tSpline);
  ...
  // // displayResults(tf, out, times, goodSamples,
  // "clippedWithArbitraryErrors",
  // // tSpline);
  ...
  // // displayResults(tf, out, sampleTimes, samples, "doublePulseFlatErrors",
  // // tSpline);
  ...
  // out = tf.fit(samples, tGuesses, noise);
  // // displayResults(tf, out, sampleTimes, samples, "triplePulseFlatErrors",
  // // tSpline);
...
void displayResults(TemplateFitter& tf, TemplateFitter::Output out,
                    ...
                    TSpline3* tSpline, TSpline3* tSplineB) {
  ...
  for (int i = 0; i < nPulses; ++i) {
    ...
    cout << "total energy: " << out.scalesA[i] + out.scalesB[i] << " +/- "
         << sqrt(varEA + varEB + correlation) << std::endl;
  }
  cout << "pedestal: " << out.pedestal << " +/- "
       << sqrt(tf.getCovariance(3 * nPulses, 3 * nPulses)) << endl;
  ...
  //   return p[6] + p[1] * tSpline->Eval(x[0] - p[0]) +
  //          p[3] * tSpline->Eval(x[0] - p[2]) +
  ...
  //     15, yMin + (yMax - yMin) * 0.9, 29, yMin + (yMax - yMin) * 0.5));
  // txtbox->SetFillColor(kWhite);
  // func->SetParameter(6, out.pedestal);
  // for (int i = 0; i < nPulses; ++i) {
  //   func->SetParameter(2 * i, out.times[i]);
  //   txtbox->AddText(Form("t_{%i}: %.3f #pm %.3f", i + 1, out.times[i],
  ...
  //   for (int i = 0; i < nPulses; ++i) {
  //     components.emplace_back(new TF1("fitFunc", templateFunction, 0, 30,
  //     7));
