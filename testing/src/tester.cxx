...
void displayResults(TemplateFitter& tf, TemplateFitter::Output out,
                    ...
                    string title, const unique_ptr<TSpline3>& tSpline);

int main() {
  new TApplication("app", 0, nullptr);

  // construct
  TFile splineF("../template0.root");
  ...
  displayResults(tf, out, sampleTimes, samples, "singlePulseArbitraryNoise",
                 tSpline);
  ...
  timeGuess =
      max_element(goodSamples.begin(), goodSamples.end()) - goodSamples.begin();
  ...
  displayResults(tf, out, times, goodSamples, "clippedWithArbitraryErrors",
                 tSpline);
  ...
  vector<double> tGuesses = {max - 1, max + 2};
...
void displayResults(TemplateFitter& tf, TemplateFitter::Output out,
                    ...
                    const unique_ptr<TSpline3>& tSpline) {
