...
TemplateFitter::Output TemplateFitter::doFit(const std::vector<double>& timeGuesses){
  ...
  while(true){
    ...
    for( fitMatrices& m : matrices_){
      m.Hess.bottomRightCorner(nPulses + 1, nPulses + 1) = m.T * m.T.transpose();

      m.b = m.T * m.pVect;

      m.b = m.Hess.bottomRightCorner(nPulses + 1, nPulses + 1).ldlt().solve(m.b);
      ...
      m.Hess.topLeftCorner(nPulses, nPulses) = diagScales*m.Hess.topLeftCorner(nPulses, nPulses)*diagScales;    
...
void TemplateFitter::evalTemplates(const std::vector<double>& tGuesses){
  ...
  for( fitMatrices& m : matrices_){
	...
	if( ( t > tMin_) && ( t  < tMax_) ){
	  ...
	  m.T(i, j) = (template_[low] + (template_[low+1] - template_[low])*dt)
	    *m.T.bottomRows(1)(0,j);

	  m.D(i, j) = (dTemplate_[low] + (dTemplate_[low+1] - dTemplate_[low])*dt)
	    *m.T.bottomRows(1)(0,j);

	  m.D2(i, j) = (d2Template_[low] + (d2Template_[low+1] - d2Template_[low])*dt)
	    *m.T.bottomRows(1)(0,j);
	}
