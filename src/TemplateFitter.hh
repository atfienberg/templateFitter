...
class TemplateFitter {
 public:
  ...
  template <typename sampleType, typename timeType = double,
            typename errorType = double>
  Output fit(const std::vector<sampleType>& trace, timeType timeGuesses,
             errorType error = 1.0) {
  ...
  Output discontiguousFit(const std::vector<sampleType>& trace,
    ...
    matrices_[0].sampleTimes.resize(
        std::count_if(sampleTimes.begin(), sampleTimes.end(), isEven));
    matrices_[1].sampleTimes.resize(sampleTimes.size() -
                                    matrices_[0].sampleTimes.size());
    ...
    for (std::size_t i = 0; i < trace.size(); ++i) {
      ...
      matrices_[evenOrOdd].pVect(nextIndex[evenOrOdd]) =
          trace[i] *
          matrices_[evenOrOdd].T.bottomRows(1)(0, nextIndex[evenOrOdd]);
      nextIndex[evenOrOdd]++;
    }
